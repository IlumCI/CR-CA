...
export class ActionRunner {
  ...
  #mandate?: Mandate;
  #budgetConsumed: { tokens: number; time: number; cost: number } = { tokens: 0, time: 0, cost: 0 };
  ...
  /**
   * Set mandate for constraint enforcement.
   */
  setMandate(mandate: Mandate): void {
    this.#mandate = mandate;
    this.#budgetConsumed = { tokens: 0, time: 0, cost: 0 };
   ...
   */
  async validateMandateConstraints(action: BoltAction): Promise<{ valid: boolean; error?: string }> {
    if (!this.#mandate) {
      return { valid: true }; // No mandate, no constraints
    }
    ...
    if (totalFiles >= constraints.maxFiles) {
      return {
        valid: false,
        error: `Maximum file limit (${constraints.maxFiles}) exceeded. Current: ${totalFiles}`,
      };
    }
    ...
    if (action.type === 'file') {
      ...
      if (fileSize > constraints.maxFileSize) {
        return {
          valid: false,
          error: `File size (${fileSize} bytes) exceeds maximum (${constraints.maxFileSize} bytes)`,
        };
      }

      // Check file extension constraint
      if (constraints.allowedFileExtensions && constraints.allowedFileExtensions.length > 0) {
        const fileExt = action.filePath.split('.').pop()?.toLowerCase();
        if (!fileExt || !constraints.allowedFileExtensions.includes(fileExt)) {
          return {
            valid: false,
            error: `File extension .${fileExt} is not allowed. Allowed: ${constraints.allowedFileExtensions.join(', ')}`,
          };
        }
      }
    }
    ...
    if (action.type === 'shell' && constraints.noNetwork) {
      const networkCommands = ['curl', 'wget', 'fetch', 'http', 'https', 'npm install', 'yarn add', 'pnpm add'];
      const hasNetworkCommand = networkCommands.some((cmd) => action.content.toLowerCase().includes(cmd.toLowerCase()));
      
      // Allow npm install if it's for allowed packages only
      if (hasNetworkCommand && action.content.includes('npm install')) {
        if (constraints.allowedPackages && constraints.allowedPackages.length > 0) {
          ...
          if (packageMatch) {
            for (const match of packageMatch) {
              const packageName = match.replace('npm install', '').trim().split('@')[0];
              if (!constraints.allowedPackages.some((allowed) => packageName.includes(allowed))) {
                return {
                  valid: false,
                  error: `Package ${packageName} is not in allowed packages list`,
                };
              }
            }
          }
          ...
          return {
            valid: false,
            error: 'Network access is disabled, but npm install requires network',
          };
        }
      } else if (hasNetworkCommand) {
        return {
          valid: false,
          error: 'Network access is disabled for this mandate',
        };
      }
    }
    ...
    if (action.type === 'shell' && action.content.includes('npm install')) {
      ...
      if (depCount > constraints.maxDependencies) {
        return {
          valid: false,
          error: `Dependency count (${depCount}) exceeds maximum (${constraints.maxDependencies})`,
        };
      }
    }
      ...
      for (const pattern of constraints.forbiddenPatterns) {
        try {
          ...
          if (regex.test(action.content)) {
            return {
              valid: false,
              error: `Action content matches forbidden pattern: ${pattern}`,
            };
          }
        } catch {
          // Invalid regex, skip
  ...
  checkBudget(): boolean {
    if (!this.#mandate) {
      return true; // No mandate, no budget limit
    }
   ...
   */
  trackBudgetConsumption(tokens: number = 0, time: number = 0, cost: number = 0): void {
    this.#budgetConsumed.tokens += tokens;
    this.#budgetConsumed.time += time;
    this.#budgetConsumed.cost += cost;
  }

  /**
   * Get current budget consumption.
   */
  getBudgetConsumption(): { tokens: number; time: number; cost: number } {
  ...
  async runAction(data: ActionCallbackData, isStreaming: boolean = false) {
    ...
    // Validate mandate constraints before execution
    const validation = await this.validateMandateConstraints(data.action);
    if (!validation.valid) {
      logger.warn(`Action ${actionId} blocked by mandate constraints: ${validation.error}`);
      this.#updateAction(actionId, {
        status: 'failed',
        error: validation.error || 'Mandate constraint violation',
      });
      this.onAlert?.({
        type: 'error',
        title: 'Mandate Constraint Violation',
        description: validation.error || 'Action violates mandate constraints',
        content: `Action: ${data.action.type}\n${validation.error}`,
      });
      return;
    }
    ...
    try {
      switch (action.type) {
        ...
        case 'file': {
          ...
          // Track file creation/modification for mandate
          if (this.#mandate) {
            const filePath = action.filePath;
            try {
              const webcontainer = await this.#webcontainer;
              await webcontainer.fs.readFile(filePath);
              // File exists, it's a modification
